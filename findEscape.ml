...
let rec traverseVar env d s =
  ...
  | A.SimpleVar (sym, _) ->
    (match S.look env sym with
     ...
     | None -> ErrorMsg.impossible ("Variable entry not found for " ^ (S.name sym) ^ " while calculating escpaes"))
...
and traverseExp env d s =
  ...
  | A.SeqExp lst ->
    List.iter (fun (exp, _) -> traverseExp env d exp) lst
