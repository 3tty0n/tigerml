...
let getResult () = !frags
let addFrag frag = frags := !frags @ [ frag ]
let init () = frags := []
...
let unNx = function Ex e -> T.EXP e | Cx _ as c -> T.EXP (unEx c) | Nx s -> s

let unCx = function
  | Cx genstm -> genstm
  | Nx _ -> ErrorMsg.impossible "unCx(Nx _) should neber need to be translated"
  ...
  let _, processed_fields =
    List.fold_left
      (fun (idx, fs) field ->
        ( idx + 1,
          T.MOVE
            ( T.MEM (T.BINOP (T.PLUS, T.TEMP r, T.CONST (idx * Frame.wordsize))),
              unEx field )
          :: fs ))
      (0, []) fields
  in
...
let computeDeclaringFrameAddress (decLevel, level) =
  let rec findFrame currLevel currAddr =
    ...
    if levelEq decLevel currLevel then (* we are in the correct frame *)
      currAddr
    else
      (* the frame we want is further up the chain *)
      let slAccess =
        match Frame.formals currFrame with
        | hd :: _ -> hd
        | [] ->
            ErrorMsg.impossible
              "found a frame with no formals - there should always be at least \
               one formal for the static link"
      in
      ...
      let parentLevel =
        ...
        | None ->
            ErrorMsg.impossible
              "followed static links to the outermost level. this should never \
               happen"
      in
...
let callExp (label, fun_level, call_level, args) =
  ...
  let parent_level =
    ...
    | Some level -> level
    | None ->
        ErrorMsg.impossible "called a function with no enclosing parent level"
  in
  let call =
    let declaring_frame_addr =
      computeDeclaringFrameAddress (parent_level, call_level)
    in
    ...
    (T.ESEQ
       ( seq
           [
             ...
             unNx (callStdlibExp (Temp.namedlabel "exit", [ Ex (T.CONST 1) ]));
             T.LABEL ok_label;
           ],
         T.MEM
           (T.BINOP
