...
exp :
  ...
  | LPAREN exp SEMICOLON exp expseq_ RPAREN
      { A.SeqExp (($2, leftPos 2) :: ($4, leftPos 4) :: $5) }
  | IF exp THEN exp
      { A.IfExp {test=$2; then'=$4; else'=None; pos=left ()} }
  | IF exp THEN exp ELSE exp
      { A.IfExp {test=$2; then'=$4; else'=Some $6; pos=left ()} }
  | WHILE exp DO exp
      { A.WhileExp {test=$2; body=$4; pos=left ()} }
  | FOR ID ASSIGN exp TO exp DO exp
      { A.ForExp {var=newSym $2; escape=ref true; lo=$4; hi=$6; body=$8; pos=left ()} }
  | LET decs IN expseq END
      { A.LetExp {decs=$2; body=A.SeqExp $4; pos=left ()} }
  | /* simple expressions */
    lvalue {A.VarExp $1}
  | LPAREN RPAREN
      { A.SeqExp [] }
  ...
  | MINUS exp %prec UMINUS
      { A.OpExp {left=A.IntExp 0; oper=A.MinusOp; right=$2; pos=left ()} }
  | STRING
      { A.StringExp ($1, left ()) }
  | BREAK
      { A.BreakExp (left ()) }
...
vardec :
  | VAR ID ASSIGN exp
      { A.VarDec {name=newSym $2; escape=ref true; typ=None; init=$4; pos=leftPos 2} }
  | VAR ID COLON ID ASSIGN exp
      { A.VarDec {name=newSym $2; escape=ref true; typ=Some (newSym $4, leftPos 4); init=$6; pos=leftPos 2} }
  ;

fundec :
  | FUNCTION ID LPAREN tyfields RPAREN EQ exp fundec_
      { A.FunctionDec ({name=newSym $2; params=$4; result=None; body=$7; pos=left ()} :: $8) }
  | FUNCTION ID LPAREN tyfields RPAREN COLON ID EQ exp fundec_
      { A.FunctionDec ({name=newSym $2; params=$4; result=Some (newSym $7, leftPos 7); body=$9; pos=left ()} :: $10) }
  | FUNCTION ID LPAREN error RPAREN EQ exp fundec_
      { error 4 (badDecl "parameter list"); A.FunctionDec $8 }
  | FUNCTION ID LPAREN error RPAREN COLON ID EQ exp fundec_
      { error 4 (badDecl "parameter list"); A.FunctionDec $10 }
  ;

fundec_ :
    /* empty */ { [] }
  | FUNCTION ID LPAREN tyfields RPAREN EQ exp fundec_
      { {name=newSym $2; params=$4; result=None; body=$7; pos=left ()} :: $8 }
  | FUNCTION ID LPAREN tyfields RPAREN COLON ID EQ exp fundec_
      { {name=newSym $2; params=$4; result=Some (newSym $7, leftPos 7); body=$9; pos=left ()} :: $10}
  | FUNCTION ID LPAREN error RPAREN EQ exp fundec_
      { error 4 (badDecl "parameter list"); $8 }
  | FUNCTION ID LPAREN error RPAREN COLON ID EQ exp fundec_
      { error 4 (badDecl "parameter list"); $10 }
  ;
expseq :
    /* empty */ { [] }
