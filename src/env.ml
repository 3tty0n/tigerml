...
type access = unit
type ty = T.ty [@@deriving show]

type enventry =
  | VarEntry of { access : Translate.access; ty : ty; const : bool }
  | FunEntry of {
...
let base_tenv : ty S.table =
  let base_types = [ ("int", T.INT); ("string", T.STRING) ] in
  List.fold_left
    (fun tbl (name, el) -> S.enter tbl (S.symbol name) el)
    S.empty base_types
...
let base_venv : enventry S.table =
  ...
  in
  List.fold_left
    (fun tbl (name, el) -> S.enter tbl name el)
    S.empty
