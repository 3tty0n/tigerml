...
module RistVGen : CODEGEN = struct
  ...
  let codegen frame stm =
    ...
    let rec munch_stm = function
      ...
      | T.EXP exp ->
        ignore (munch_exp exp)
      | stm ->
        ErrorMsg.impossible @@ "Unmatched pattern " ^ (Tree.show_stm stm)
  ...
  let%test_unit "test_assem" =
    ...
    let frame = Frame.newFrame { name = Temp.namedlabel "main"; formals = [] } in
    let input_string = {|
      1 + 1
|} in
